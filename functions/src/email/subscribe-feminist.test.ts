...
describe('Subscribe to mailchimp', () => {
	test('should not work if no optin', async done => {
		const wraped = testEnv.wrap(api.subscribed_to_newsletter);
		const new_body = body;

		new_body.optin = false;
		const snap = testEnv.firestore.makeDocumentSnapshot(new_body, 'events/{eventId}/feminists/{feministId}');
	...
	test('should check e-mail', async done => {
		...
		const snap = testEnv.firestore.makeDocumentSnapshot(body, 'events/{eventId}/feminists/{feministId}');
		const result = await wraped(snap);
	...
	test('should work parfaitly', async done => {
		...
		const snap = testEnv.firestore.makeDocumentSnapshot(body, 'events/seed-event-waiting_leader/feminists/alreadyAddToMailchimp');
